...
export class UsersServices {
  ...
  GetUsers(Page: number, PageSize: Number): Observable<BaseResponsePagination<GetUsersCommand[]>> {
    return this.http.get<BaseResponsePagination<GetUsersCommand[]>>(
      `${this.url}/api/UserMangament/GetUsers?culture=ar-LY&Page=${Page}&PageSize=${PageSize}`
    );
  }
}
