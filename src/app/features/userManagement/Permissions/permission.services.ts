...
export class PermissionServices {
  ...
  GetGroupsMenu(Page: number, PageSize: number, IsActive: 1): Observable<BaseResponsePagination<getGroupsMenuCommand[]>> {
    return this.http.get<BaseResponsePagination<getGroupsMenuCommand[]>>(
      `${this.url}/api/Groups/GetGroupsMenu?IsActive=${IsActive}&culture=ar-LY&Page=${Page}&PageSize=${PageSize}`
    );
  }
  GetAllGroup(Page: number, PageSize: number): Observable<BaseResponsePagination<getAllGroup[]>> {
    return this.http.get<BaseResponsePagination<getAllGroup[]>>(
      `${this.url}/api/Groups/GetAllGroup?culture=ar-LY&Page=${Page}&PageSize=${PageSize}`
    );
  }
